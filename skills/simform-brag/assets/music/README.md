...
These files are generated by `skills/simform-brag/scripts/analyze_music_cues.py` and are intended for `/simform-brag` planning. Normal skill runs should read the presets when present and continue without beat/cue sync when they are missing.
